...
![Git Flow](git_flow.png "Git Flow")
...
**Snapshot**: 해당 시점에서의 폴더나 파일 같은 작업 디렉토리의 변경 상태를 의미하며, 스테이징이나 커밋을 할 때 이전 스냅샷과의 변경점이 기록된다  
**Branch**: 분기점으로, 이후에 충돌하는 작업물이 없는 파일에 한해서 메인 브랜치와 다시 병합할 수 있다  
**HEAD**: 해당 브랜치에서 현재 작업 중인 커밋을 가리키며, @로 대체해서 표기할 수 있다  
...
**Rebase**: 현재 브랜치의 분기점 커밋을 해당 브랜치의 헤드로 이동시켜서 재배치  
**Merge**: 현재 작업 공간을 해당 브랜치의 헤드의 커밋과 현재 브랜치의 헤드 커밋을 합친 상태로 변경  
...
* docs: 주석이나 문서를 변경
* style: 기능에 영향이 없는 선에서 단순하게 코드 구조를 변경
* refactor: 추후 더 복잡한 기능 구현을 위해서 기존 코드의 재사용성을 높이는 코드 구조로 변경
* rename: 파일 이름을 변경
* remove: 파일을 삭제
* test: 테스트 코드를 변경
* build: 빌드 파일을 변경
...
* `#`로 주석 작성
* `!`로 시작하는 패턴의 파일을 의미
* `/`로 시작하면 `최상위 경로에서부터`를 의미
* `/`로 끝나면 폴더를 의미 // 아니라면 폴더 또는 파일을 의미
...
* 문서의 줄 바꿈을 커밋 시에는 LF 형식으로 자동 변환하고, 체크아웃 시에는 변환하지 않는 옵션: `input`(권장)
...
저장소와 작업 공간에서 파일 삭제: `git rm "<file_name>"` // 파일이 스테이지 영역에 있어도 강제로 삭제하는 옵션: `-f`  
스테이지 영역에서만 파일 삭제: `git rm --cached "<file_name>"`  
해당 파일을 가장 최근의 커밋의 상태로 복원: `git restore "<file_name>"`  
해당 파일을 해당 커밋의 상태로 복원: `git restore --source "<commit_hash>" "<file_name>"`  
해당 파일을 스테이지 영역에서 삭제: `git restore --staged "<file_name>"`
...
### 5. 커밋 정보 비교(변화가 생긴 파일 목록만 표기하는 옵션: `--name-only`)
...
현재 커밋에서부터 새로운 브랜치 생성: `git branch "<branch_name>"`
해당 커밋에서부터 새로운 브랜치 생성: `git branch "<branch_name>" "<commit_hash>"`  
브랜치 리스트 보기: `git branch`

* 원격 저장소의 브랜치 리스트 보기 옵션: `-r`, `--remotes`
* 로컬 저장소와 원격 저장소의 브랜치 리스트 보기 옵션: `-a`, `-all`
* 브랜치 이름 변경 옵션: `-m`, `--move`
...
병합 도중 충돌이 발생한 경우 병합을 취소: `git merge --abort` // 이미 병합을 완료했다면 `git reset`을 이용해야 함

* 편집기를 열기 않고 제목을 추가하는 옵션: `-m "<commit_message_header>"`
* 편집기를 열기 않고 제목과 본문을 추가하는 옵션: `-m "<commit_message_header>" -m "<commit_message_body>"`
* 병합될 브랜치의 모든 커밋들을 하나의 커밋으로 합치는 옵션: `--squash`
* Fast-forward가 가능한 경우라도 하지 않게 하는 옵션: `--no-ff` // 커밋 기록과 브랜치가 계속 유지되어야 할 경우에 유용

현재 브랜치의 기반 커밋을 해당 브랜치 헤드로 재배치: `git rebase "<branch_name>"`  
현재 브랜치의 최근 n 개의 커밋을 수정하기 위해 설정 문서와 상호작용 : `git rebase HEAD~n`

### 8. 원격 저장소

원격 저장소 연결: `git remote add "<remote_repo_name>" "<remote_repo_url>"` // 깃허브 레포지토리가 비공개일 경우 `https://` 뒤에 `<깃허브 유저 이름>@`을 추가한다  
추가한 원격 저장소 목록 보기: `git remote` // 상세히 보기 옵션: `-v`  
원격 저장소 url 얻기: `git remote get-url "<remote_repo_name>"`  
원격 저장소 url 설정하기: `git remote set-url "<remote_repo_name>" "<remote_repo_url>"`  
...
원격 저장소로 해당 브랜치 올리기: `git push "<remote_repo_name>" "<local_branch_name>"`

* 모든 브랜치를 올리는 옵션: `--all`
* 강제로 올리는 옵션: `-f`, `--force`
* 로컬 저장소와 원격 저장소를 계속 연동시키는 옵션: `-u`

원격 저장소로 모든 브랜치 올리기: `git push "<remote_repo_name>" --all`  
원격 저장소에서 해당 브랜치 내려받기: `git fetch "<remote_repo_name>" "<remote_branch_name>"`  
원격 저장소에서 해당 브랜치를 내려받는 동시에 현재 브랜치 헤드에 병합: `git pull "<remote_repo_name>" "<remote_branch_name>"`

### 9. 태그(한 번 지정한 태그는 수정이 불가능하지만, 새로운 태그를 추가하거나 삭제할 수 있다)
...
* 커밋 해시 대신에 태그를 사용하면 태그가 커밋 해시로 자동 역참조 된다
...
* 따라서 커밋 해시 대신에 주석 달린 태그를 사용하려면 역참조 과정을 한 번 거쳐야 한다
* 주석 달린 태그에 대한 역참조: `<annotated_tag_name>^{}`
...
원격 저장소의 태그 목록 보기: `git ls-remote "<remote_repo_name>" --tags`  
...
원격 저장소에서 해당 태그 내려받기: `git fetch "<remote_repo_name>" "<tag_name>"`  
원격 저장소에서 모든 태그 내려받기: `git fetch "<remote_repo_name>" --tags`

### 10. 작업 임시 저장(추적 상태인 파일에 한해서만 가능)
...
깃 프로젝트에 서브 모듈 추가: `git submodule add "<submodule_url>" "<submodule_path>"`  
서브 모듈 상태 확인: `git submodule status`  
메인 프로젝트에 등록된 서브 모듈을 최신 정보로 초기화하고 다운로드: `git submodule init && git submodule update` // 메인 프로젝트를 클론해도 서브 모듈은 따로 다운로드 받아야 한다  
서브 모듈 삭제: `git submodule deinit -f "<submodule_path>" && git rm -f "<submodule_path>"` // 서브 모듈 캐시 위치: `.git/modules/<submodule_name>`
